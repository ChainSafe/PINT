...
fn feed_creation_and_mapping_should_work() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PriceFeed::get_price(PINTAssetId::get()),
            Error::<Test>::AssetPriceFeedNotFound
        );

        // map feed 0 to PINT
        assert_ok!(PriceFeed::track_asset_price_feed(
...
fn non_admin_cannot_map_feeds() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PriceFeed::track_asset_price_feed(Origin::signed(1), PINTAssetId::get(), 0),
            BadOrigin
        );
    })
}
...
fn cannot_get_price_pair_for_feed_without_valid_round() {
    new_test_ext().execute_with(|| {
        ...
        assert_noop!(
            PriceFeed::get_price(ASSET_X_ID),
            Error::<Test>::InvalidFeedValue
        );
    })
}
