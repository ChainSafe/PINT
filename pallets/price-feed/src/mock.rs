...
frame_support::construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic,
	{
		...
		OrmlOracle: orml_oracle::<Instance1>,
		OracleOperatorMembership: pallet_membership::{Pallet, Call, Storage, Event<T>, Config<T>},
	}
);
...
impl orml_oracle::Config<PintDataProvider> for Test {
	type Event = Event;
	type OnNewData = ();
	type CombineData = orml_oracle::DefaultCombineData<Test, MinimumCount, ExpiresIn, PintDataProvider>;
	type Time = Timestamp;
...
impl pallet_membership::Config for Test {
	type Event = Event;
	type AddOrigin = EnsureSignedBy<AdminAccountId, AccountId>;
	type RemoveOrigin = EnsureSignedBy<AdminAccountId, AccountId>;
	type SwapOrigin = EnsureSignedBy<AdminAccountId, AccountId>;
	type ResetOrigin = EnsureSignedBy<AdminAccountId, AccountId>;
	type PrimeOrigin = EnsureSignedBy<AdminAccountId, AccountId>;
