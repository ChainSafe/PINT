...
fn can_set_voting_period() {
	new_test_ext_without_members().execute_with(|| {
		...
		assert_noop!(Committee::set_voting_period(Origin::root(), WEEKS), pallet::Error::<Test>::InvalidVotingPeriod);
		assert_noop!(Committee::set_voting_period(Origin::root(), WEEKS), pallet::Error::<Test>::InvalidVotingPeriod);
		assert_ok!(Committee::set_voting_period(Origin::root(), WEEKS + 1));
		assert_eq!(pallet::VotingPeriod::<Test>::get(), WEEKS + 1);

		assert_noop!(
			Committee::set_voting_period(Origin::root(), 4 * WEEKS),
			pallet::Error::<Test>::InvalidVotingPeriod
		);
		assert_ok!(Committee::set_voting_period(Origin::root(), 4 * WEEKS - 1));
		assert_eq!(pallet::VotingPeriod::<Test>::get(), 4 * WEEKS - 1);
	});
}
