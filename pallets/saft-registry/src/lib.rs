...
pub mod pallet {
	...
	pub type ActiveSAFTs<T: Config> =
		StorageMap<_, Blake2_128Concat, T::AssetId, Vec<SAFTRecord<T::Balance, T::Balance>>, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn remove_saft(origin: OriginFor<T>, asset_id: T::AssetId, index: u32) -> DispatchResult {
			T::AdminOrigin::ensure_origin(origin.clone())?;
