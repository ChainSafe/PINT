...
fn deposit_ok_for_when_price_feed_unavailable() {
	...
	new_test_ext_with_balance(balance).execute_with(|| {
		...
		assert_ok!(AssetIndex::deposit(Origin::signed(ASHLEY), UNKNOWN_ASSET_ID, 1),);
	})
}
...
fn can_calculate_nav() {
	new_test_ext().execute_with(|| {
		...
		let total_pint = AssetIndex::index_token_issuance();
		assert_eq!(total_pint, saft_units + liquid_units + 1_000 * ASSET_A_PRICE_MULTIPLIER);

		let asset_volume = (a_units + 1_000) * ASSET_A_PRICE_MULTIPLIER + b_units * ASSET_B_PRICE_MULTIPLIER;
		let total_nav = AssetIndex::total_nav().unwrap();
		assert_eq!(total_nav, asset_volume / total_pint);
	})
}
...
fn can_withdraw() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(
			total_pint,
			a_tokens + b_tokens + 1_000 * ASSET_A_PRICE_MULTIPLIER + 2_000 * ASSET_B_PRICE_MULTIPLIER
		);
		let user_pint = AssetIndex::index_token_balance(&ASHLEY);
		assert_eq!(user_pint, 1_000 * ASSET_A_PRICE_MULTIPLIER + 2_000 * ASSET_B_PRICE_MULTIPLIER);
		...
		assert_eq!(total_value, asset_a_units * ASSET_A_PRICE_MULTIPLIER + asset_b_units * ASSET_B_PRICE_MULTIPLIER);

		// ratio of asset a to total value
		let a_proportional =
			FixedU128::checked_from_rational(asset_a_units * ASSET_A_PRICE_MULTIPLIER, total_value).unwrap();

		let b_proportional =
			FixedU128::checked_from_rational(asset_b_units * ASSET_B_PRICE_MULTIPLIER, total_value).unwrap();
		...
		// the payout
		assert_ok!(AssetIndex::add_saft(&ADMIN_ACCOUNT_ID, SAFT_ASSET_ID, 1_000, 2_000));
		...
		assert_eq!(
			pending.assets.iter().find(|x| x.asset == ASSET_A_ID).expect("asset should be present"),
			&AssetWithdrawal { asset: ASSET_A_ID, state: RedemptionState::Unbonding, units: a_redeemed_units }
		);

		assert_eq!(
			pending.assets.iter().find(|x| x.asset == ASSET_B_ID).expect("asset should be present"),
			&AssetWithdrawal { asset: ASSET_B_ID, state: RedemptionState::Unbonding, units: b_redeemed_units }
		);

		// make sure the holding balance is updated
		assert_eq!(AssetIndex::index_free_asset_balance(ASSET_A_ID), asset_a_units - a_redeemed_units);
		assert_eq!(AssetIndex::index_free_asset_balance(ASSET_B_ID), asset_b_units - b_redeemed_units);
	})
}
