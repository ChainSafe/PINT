...
//! ## Overview
//!
//! Transaction payment pallet is responsible for charge fee and tip in
//! different assets
...
pub mod module {
	...
	pub trait Config: frame_system::Config {
		...
		#[pallet::constant]
		type DefaultFeeSwapPathList: Get<Vec<Vec<AssetId>>>;
	...
	PalletBalanceOf<T>: FixedPointOperand,
{
	...
	pub fn ensure_can_charge_fee(who: &T::AccountId, fee: PalletBalanceOf<T>, reason: WithdrawReasons) {
		...
		if !native_is_enough {
			...
			let native_currency_id = T::NativeAssetId::get();
			let default_fee_swap_path_list = T::DefaultFeeSwapPathList::get();
			let fee_swap_path_list: Vec<Vec<AssetId>> =
